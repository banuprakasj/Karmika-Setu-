...
export default function PublicProfile({ setuId }: PublicProfileProps) {
  ...
  const handlePrint = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Shramika Setu Card</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="title">Shramika Setu Card</div>
          ...
          <div class="footer">Document digitally signed by Karmika Setu Identity Protocol — Certificate ID: KS-${profile.setuId}-${Date.now()}</div>
        </body>
      </html>
    `;
  ...
  if (error || !profile) {
    return (
      <div className="h-screen bg-[#F5F5F0] flex flex-col items-center justify-center p-8 text-center">
        ...
        <p className="text-brand-ink/50 mb-8 max-w-xs">{error || 'This Karmika ID does not exist in the verified registry.'}</p>
      ...
      <div className="max-w-5xl mx-auto space-y-8 document-container">
        ...
        <header className="flex flex-col md:flex-row justify-between items-center no-print px-4 gap-4 mb-2">
          <div className="flex items-center gap-3">
             ...
             <div>
                <span className="font-display font-black uppercase tracking-tight text-xl text-brand-ink">Shramika<span className="text-brand-primary">Setu</span> Card</span>
            ...
            <div className="flex-1 space-y-6">
              ...
              <div className="max-w-2xl">
                 <p className="text-sm font-medium leading-relaxed italic text-brand-ink/70">
                    "Verified professional with over {profile.totalDaysWorked} days of documented history in {processedSkills[0]?.name || 'the informal sector'}. Demonstrated consistent reliability and skill mastery as verified by multiple independent contractors and the Karmika Setu identity protocol."
                 </p>
              </div>
          ...
          <div className="mt-24 space-y-12">
             <div className="bg-brand-ink text-white p-12 rounded-[3.5rem] flex flex-col md:flex-row justify-between items-center gap-10 relative overflow-hidden print:rounded-3xl print:p-8 print:mt-12">
                ...
                <div className="relative z-10 space-y-6 flex-1">
                  ...
                  <p className="text-white/40 text-sm max-w-xl leading-relaxed font-medium">
                    This document serves as an official authenticated professional record. All financial data and service periods have been strictly verified through biometric cross-referencing and multi-signatory confirmation by registered contractors and organizations within the Karmika Setu decentralized ecosystem. Any alteration of this document invalidates its digital signature.
                  </p>
             ...
             <div className="flex flex-col md:flex-row justify-between items-center opacity-30 text-[8px] font-black uppercase tracking-[0.5em] gap-8 border-t border-brand-ink/10 pt-12">
                ...
                <div className="text-center">
                   © 2026 KARMIKA SETU — NATIONAL DATA INFRASTRUCTURE
                </div>
        ...
        <footer className="text-center text-[10px] font-black uppercase tracking-[0.3em] opacity-40 py-12 no-print flex flex-col items-center gap-4">
          ...
          End of Official Verified Professional Identity Record [KARMIKA ID: {profile.setuId}]
        </footer>
      </div>
    </div>
  );
