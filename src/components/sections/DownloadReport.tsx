...
export default function DownloadReport({ profile, entries, t }: DownloadReportProps) {
  ...
  const handleDownload = () => {
    ...
    if (activeReportType === 'pitch') {
       ...
       doc.text('KARMIKA SETU', 105, 50, { align: 'center' });
       doc.setFontSize(16);
       ...
       doc.save(`karmika-setu-pitch-deck.pdf`);
       return;
    }
    ...
    if (activeReportType === 'skills') {
       ...
       doc.setFontSize(10);
       doc.text('Certified by Karmika Setu Workforce Analysis', 105, 28, { align: 'center' });
    ...
    doc.setFontSize(22);
    doc.text('KARMIKA SETU - WORK CERTIFICATE', 105, 20, { align: 'center' });
    // ... rest of standard report logic ...
    doc.setFontSize(12);
    doc.text(`Generated on: ${new Date().toLocaleDateString()}`, 20, 50);
    doc.text(`Worker Name: ${user.name}`, 20, 60);
    doc.text(`Karmika Setu ID: ${user.setuId}`, 20, 70);
    ...
    doc.save(`karmika-work-cert-${user.setuId}.pdf`);
  };
  ...
  const handlePrint = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Karmika Setu - Work Certificate</title>
          <style>
        ...
        <body>
          ...
          <div class="footer">This is an official verifiable work certificate from the Karmika Setu registry. Digital ID: ${profile.setuId}</div>
        </body>
      </html>
    `;
  ...
  const shareToWhatsapp = () => {
    const text = encodeURIComponent(`Check out my verified Karmika Setu Work Report: ${window.location.origin}/view/${user.setuId}`);
    window.open(`https://wa.me/?text=${text}`, '_blank');
  };

  const shareEmail = () => {
    const subject = encodeURIComponent('Verified Work Certificate - Karmika Setu');
    const body = encodeURIComponent(`Hello,\n\nPlease find my verified work history report at: ${window.location.origin}/view/${user.setuId}\n\nGenerated by Karmika Setu.`);
  ...
  const shareGeneral = async () => {
    if (navigator.share) {
      await navigator.share({
        title: 'Karmika Setu Work Report',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl p-8 card-shadow border border-brand-ink/5 space-y-6">
            ...
            {activeReportType === 'idcard' ? (
              <div className="aspect-[1.586/1] bg-[#141414] rounded-xl p-8 shadow-inner overflow-hidden flex flex-col text-white relative">
                <div className="absolute top-0 right-0 w-32 h-32 bg-white opacity-5 rounded-bl-[100px]" />
                <div className="flex justify-between items-start relative z-10">
                  <div>
                    <h4 className="text-2xl font-black tracking-widest">SHRAMIKA SETU</h4>
                    <p className="text-[10px] font-bold tracking-[0.2em] opacity-60">VERIFIED DIGITAL IDENTITY</p>
                  </div>
                  {user.isVerified && (
                    <div className="px-3 py-1 bg-[#5A5A40] text-xs font-bold rounded">VERIFIED</div>
                  )}
                </div>
              ...
              <div className="aspect-[1/1.414] bg-brand-paper rounded-xl p-8 shadow-inner overflow-hidden flex flex-col">
                <div className="text-center space-y-2 mb-8 border-b border-brand-ink/10 pb-6">
                  ...
                  <h4 className="text-sm font-bold uppercase tracking-widest opacity-50">Karmika Setu</h4>
