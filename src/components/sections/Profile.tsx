...
import { User, Phone, MapPin, Calendar, Wallet, Award, Download, Share2, Mail, X, Save, Copy, ShieldCheck, Camera, Loader2, CheckCircle2, Smartphone } from 'lucide-react';
...
export default function Profile({ profile, t, onUpdate, onNavigate }: ProfileProps) {
  ...
  const downloadQR = () => {
    ...
    if (canvas) {
      ...
      link.download = `Karmika_Setu_QR_${user.setuId}.png`;
      link.href = url;
      link.click();
    }
  };
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title: `Karmika Setu Profile: ${user.name}`,
