...
export default function WorkHistory({ entries, t }: WorkHistoryProps) {
  ...
  const handleExportCSV = () => {
    ...
    link.setAttribute("download", `Karmika_Work_History_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handlePrint = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Karmika Setu - Work History</title>
          <style>
        ...
        <body>
          <h1>KARMIKA SETU - OFFICIAL WORK RECORD</h1>
          ...
          `).join('')}
          <div class="footer">This is an automated verifiable digital record from the Karmika Setu Protocol.</div>
        </body>
      </html>
    `;
  ...
  const downloadIndividualEntry = (e: WorkEntry) => {
    const content = `
KARMIKA SETU - WORK VERIFICATION RECORD
---------------------------------------
...
Verified by Karmika Setu Digital Infrastructure.
    `;
