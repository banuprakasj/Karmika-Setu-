                  ...
                  {filteredWorkers.map((worker) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 text-right text-[10px] font-black uppercase tracking-widest text-brand-muted">
                        <button
                          disabled={isActionLoading || worker.status === 'working' || worker.status === 'offline'}
                          onClick={() => setHiringWorker(worker)}
                          className={cn(
                            "px-4 py-2 text-white rounded-xl transition-all flex items-center gap-2 ml-auto",
                            (isActionLoading || worker.status === 'working' || worker.status === 'offline')
                              ? "bg-brand-muted cursor-not-allowed opacity-50"
                              : "bg-brand-ink hover:bg-brand-primary group-hover:scale-105"
                          )}
                        >
                          <Send size={12} />
                          {t.hireNow}
