...
export default function OrgDashboard({ profile }: OrgDashboardProps) {
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!setuSnap.exists()) {
        throw new Error('Karmika ID not found. Please double check.');
      }
  ...
  const handleDownloadWorkerPDF = (worker: UserProfile, history: WorkEntry[]) => {
    const doc = new jsPDF();
    doc.setFontSize(22);
    doc.text('KARMIKA SETU - WORKER PROFILE', 105, 20, { align: 'center' });
    doc.setFontSize(14);
  ...
  const handlePrint = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Karmika Setu - Worker Profile</title>
          <style>
        ...
        <body>
          <div class="header">
            <div class="title">Karmika Setu Worker Record</div>
          ...
          <div class="footer">This report is an automated digital verification of the worker's history from the Karmika Setu registry.</div>
        </body>
      </html>
    `;
  ...
  const handleDownloadESG = () => {
    ...
    doc.text('Karmika Setu Organization Portal', 105, 38, { align: 'center' });
      ...
      <div className="bg-white p-2 rounded-[2rem] border border-brand-ink/5 shadow-sm flex flex-col md:flex-row gap-2">
        <form onSubmit={handleLookup} className="flex-1 flex items-center relative">
          ...
          <input 
            ...
            placeholder="Search Karmika by ID (e.g. SS-XXXXX)"
          ...
          <div className="bg-brand-ink rounded-[2.5rem] p-10 text-white relative overflow-hidden min-h-[400px] flex flex-col justify-between">
            <div className="relative z-10">
              ...
              <p className="text-white/60 text-sm mb-8 leading-relaxed">
                Your participation in the Karmika Setu ecosystem has enabled formal credit access for over 450 unorganized sector workers this quarter.
              </p>
          ...
          >
            ...
            >
              ...
              <div className="hidden print:block mb-8 text-center border-b-2 border-brand-ink pb-6">
                <h1 className="text-4xl font-black uppercase tracking-tight mb-2">Karmika Setu - Organization Verification</h1>
