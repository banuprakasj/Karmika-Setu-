...
export default function WorkerSearch({ initialQuery }: WorkerSearchProps) {
  ...
  const handlePrint = () => {
    ...
    const content = `
      <html>
        <head>
          <title>Karmika Setu - Worker Report</title>
          <style>
        ...
        <body>
          ...
          <div class="footer">This report is an automated digital verification from the Karmika Setu registry.</div>
        </body>
      </html>
    `;
  ...
  const handleDownloadReport = (worker: UserProfile, entries: WorkEntry[]) => {
    const doc = new jsPDF();
    doc.setFontSize(22);
    doc.text('KARMIKA SETU - WORKER REPORT', 105, 20, { align: 'center' });
    doc.setFontSize(14);
          ...
          {viewMode === 'profile' && (
            <div className="bg-white p-8 rounded-[2.5rem] border border-brand-ink/5 transition-all print:p-0 print:border-none">
              ...
              <div className="hidden print:block mb-8 text-center border-b-2 border-brand-ink pb-6">
                <h1 className="text-4xl font-black uppercase tracking-tight mb-2">Karmika Setu - Official Worker Report</h1>
