...
export default function Auth({ onLogin, role, onBack, t, lang, onLanguageChange }: AuthProps) {
  ...
  const [error, setError] = useState<string | null>(() => {
    const err = localStorage.getItem('karmik_auth_error');
    if (err) {
      localStorage.removeItem('karmik_auth_error');
      return err;
    }
    return null;
  });
    ...
    try {
      if (mode === 'login') {
        ...
        if (profile && profile.role !== role) {
          localStorage.setItem('karmik_auth_error', `This account is registered as a ${profile.role}. Please use the correct portal.`);
          await signOut(auth);
          return;
        }
        ...
        {showForgotPassword ? (
          ...
          <div className="flex items-center justify-between mb-8">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-display font-black text-2xl tracking-tight leading-none uppercase text-brand-ink">Karmika Setu</h1>
