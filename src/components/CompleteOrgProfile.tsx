...
export default function CompleteOrgProfile({ roleOverride, t }: CompleteOrgProfileProps) {
      ...
      >
        <div className="p-8 text-center">
          ...
          <h1 className="font-display font-black text-2xl tracking-tight uppercase mb-2">
            {isContractor ? t?.contractorSetup || 'Contractor Setup' : t?.orgSetup || 'Organization Setup'}
          </h1>
          <p className="text-sm text-[#141414]/60 mb-8">{t?.setupDesc || 'Professional details to help verify your identity on Karmika Setu.'}</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-left">
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest opacity-40 ml-1">
                  {isContractor ? t?.businessNameLabel || 'Business / Company' : t?.orgNameLabel || 'Organization Name'}
                </label>
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-widest opacity-40 ml-1">{t?.orgTypeLabel || 'Organization Type'}</label>
                <div className="relative">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest opacity-40 ml-1">{t?.contactPersonLabel || 'Contact Person'}</label>
              <div className="relative">
            ...
            {isContractor && (
              <div className="grid md:grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest opacity-40 ml-1">{t?.phoneNumber || 'Phone Number'}</label>
                  <div className="relative">
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-widest opacity-40 ml-1">{t?.aadhaarNumber || 'Aadhaar ID'}</label>
                  <div className="relative">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold uppercase tracking-widest opacity-40 ml-1">{t?.officeAddress || 'Office Address'}</label>
              <div className="relative">
        ...
        <div className="bg-[#F5F5F0]/50 p-6 flex items-center gap-3 border-t border-[#141414]/5">
          ...
          <p className="text-[10px] uppercase font-bold tracking-wider opacity-60 leading-tight text-left">
            {t?.kycNotice || 'Organization accounts are subjected to KYC verification before accessing worker data.'}
          </p>
        </div>
      </motion.div>
    </div>
